







var lenguaje;
var velocidad;

document.getElementById('hablar').addEventListener("click",()=>{
  decir(document.getElementById("texto-decir").value)
})




  function decir(texto) {
    velocidad = document.querySelector("#particles-js > div > div > div > div:nth-child(3) > input").value
    if (document.querySelector("#select-idioma").value == "1"){
      lenguaje = "es"
    }else{
      lenguaje = "en"
    }
    if (speechSynthesis.speaking) {
        speechSynthesis.cancel();
    }
    else {
        var message = new SpeechSynthesisUtterance(texto);
        message.lang = lenguaje;
        message.rate = velocidad;
        speechSynthesis.speak(message);
    }
}


/* particles js*/


particlesJS('particles-js',
  
  {
    "particles": {
      "number": {
        "value": 60,
        "density": {
          "enable": true,
          "value_area": 800
        }
      },
      "color": {
        "value": "#800"
      },
      "shape": {
        "type": "circle",
        "stroke": {
          "width": 1,
          "color": "#800"
        },
        "polygon": {
          "nb_sides": 5
        },
        "image": {
          "src": "img/github.svg",
          "width": 100,
          "height": 100
        }
      },
      "opacity": {
        "value": 0.5,
        "random": false,
        "anim": {
          "enable": false,
          "speed": 1,
          "opacity_min": 0.1,
          "sync": false
        }
      },
      "size": {
        "value": 5,
        "random": true,
        "anim": {
          "enable": false,
          "speed": 36,
          "size_min": 0.1,
          "sync": false
        }
      },
      "line_linked": {
        "enable": true,
        "distance": 150,
        "color": "#800",
        "opacity": 0.4,
        "width": 1
      },
      "move": {
        "enable": true,
        "speed": 2,
        "direction": "none",
        "random": false,
        "straight": false,
        "out_mode": "out",
        "attract": {
          "enable": false,
          "rotateX": 600,
          "rotateY": 1200
        }
      }
    },
    "interactivity": {
      "detect_on": "canvas",
      "events": {
        "onhover": {
          "enable": false,
          "mode": "repulse"
        },
        "onclick": {
          "enable": false,
          "mode": "push"
        },
        "resize": true
      },
      "modes": {
        "grab": {
          "distance": 0,
          "line_linked": {
            "opacity": 1
          }
        },
        "bubble": {
          "distance": 400,
          "size": 40,
          "duration": 2,
          "opacity": 8,
          "speed": 3
        },
        "repulse": {
          "distance": 200
        },
        "push": {
          "particles_nb": 4
        },
        "remove": {
          "particles_nb": 2
        }
      }
    },
    "retina_detect": true,
    "config_demo": {
      "hide_card": false,
      "background_color": "#800",
      "background_image": "",
      "background_position": "50% 50%",
      "background_repeat": "no-repeat",
      "background_size": "contain"
    }
  }

);




// posicionar background de canvas


document.querySelector("#particles-js > div > div").style.position = "relative";
document.querySelector("#particles-js > div > div").style.zIndex = "1000";

document.querySelector("#particles-js > canvas").style.position = "absolute";
document.querySelector("#particles-js > canvas").style.top = "0";